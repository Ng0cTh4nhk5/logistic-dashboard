...
> **Truy cập:** [https://ng0cth4nhk5.github.io/logistic-dashboard/](https://ng0cth4nhk5.github.io/logistic-dashboard/)  
> **Lưu trữ dữ liệu:** localStorage (trình duyệt) — không cần server, không cần đăng nhập
...
2. [Truy Cập & Bắt Đầu](#2-truy-cập--bắt-đầu)
...
8. [Phụ Lục: Cài Đặt Local (Dành cho dev)](#8-phụ-lục-cài-đặt-local-dành-cho-dev)
...
> ### 🔗 [https://ng0cth4nhk5.github.io/logistic-dashboard/](https://ng0cth4nhk5.github.io/logistic-dashboard/)

**Yêu cầu:** Trình duyệt hiện đại (Chrome, Firefox, Edge, Safari). Không cần cài đặt, không cần đăng nhập.

### Lần đầu truy cập

Khi mở lần đầu, bạn sẽ thấy trang **Chào mừng** với 2 lựa chọn:

| Tuỳ chọn | Mô tả |
|-----------|-------|
| 🚀 **Start Fresh** | Bắt đầu từ đầu — nhập dữ liệu doanh nghiệp của bạn qua 6 bước (~5 phút) |
| 📦 **Load Demo Data** | Xem trước dashboard với dữ liệu mẫu ngành dầu ăn (GoldenHarvest Foods, USA) |

> 💡 **Khuyến nghị:** Nếu chưa biết ứng dụng trông như thế nào, chọn **Load Demo Data** trước. Sau đó vào **⚙️ Settings** để chỉnh sửa thành dữ liệu thật.

### Các lần truy cập tiếp theo

Dữ liệu được lưu trên trình duyệt. Khi mở lại cùng link trên cùng trình duyệt, bạn sẽ thấy **Dashboard** với dữ liệu đã nhập trước đó — không cần nhập lại.
...
Đã mô tả ở phần [Truy Cập & Bắt Đầu](#2-truy-cập--bắt-đầu) phía trên. Click **🚀 Start Fresh** để bắt đầu nhập dữ liệu.
...
| **Annual Revenue** | Không | Doanh thu hàng năm (dùng để tính % chi phí logistics) | `120,000,000,000` |
...
> 💡 **Auto-format số:** Khi nhập số lớn (Revenue, Employees), ứng dụng tự động chèn dấu phẩy phân cách hàng nghìn. Ví dụ: nhập `120000000000` → hiện `120,000,000,000`.
...
| **Price/Unit** | Giá bán mỗi đơn vị sản phẩm (auto-format) | `35,000` |
...
| **Distance** | Khoảng cách (auto-format) | `35 km` |
...
| **Cost/Trip** | Chi phí mỗi chuyến (auto-format) | `1,500,000` |
...
2. **Nhập số tiền:** Click vào ô số bên phải, nhập chi phí hàng tháng — số tự động format dấu phẩy (ví dụ: `180,000,000`)
3. **Mở rộng nhóm:** Click vào tên nhóm (▶/▼) để mở rộng/thu gọn
4. **Thêm hạng mục:** Click **`+ Add custom category`** ở cuối mỗi nhóm
5. **Sửa tên custom:** Click vào tên hạng mục tự tạo → nhập tên mới (ô có viền nét đứt)
6. **Xoá custom:** Di chuột lên hạng mục tự tạo → click nút **✕** bên phải → xác nhận xoá
7. **Monthly Total:** Badge phía trên hiển thị tổng chi phí hàng tháng, cập nhật realtime

> 💡 **Lưu ý:** Chỉ các hạng mục **tự tạo** (custom) mới có thể sửa tên/xoá. Các hạng mục mặc định (Fuel, Driver wages...) không thể xoá, chỉ có thể tắt.
...
| **Company Logo** *(tuỳ chọn)* | Upload logo công ty (PNG, JPEG, SVG, WebP — tối đa 2MB, tự resize nếu lớn) | — |
...
## 8. Phụ Lục: Cài Đặt Local (Dành cho dev)

Nếu bạn muốn chạy ứng dụng trên máy tính cá nhân (ví dụ: để phát triển hoặc chạy offline):
...
```bash
# 1. Clone source code
git clone https://github.com/Ng0cTh4nhk5/logistic-dashboard.git
...
> 💡 Xem thêm file `DEPLOY.md` để hướng dẫn deploy lên hosting.
